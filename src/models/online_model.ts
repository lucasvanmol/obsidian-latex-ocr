...
export default class ApiModel implements Model {
    ...
    async imgfileToLatex(filepath: string): Promise<string> {
        ...
        const response = await requestUrl(
            {
                url: "https://api-inference.huggingface.co/models/Norm/nougat-latex-base",
        ...
        if (response.status === 200) {
            ...
            throw new Error(`Got ${response.status}: ${response.text}`)
        }
    }
    ...
    async status() {
        ...
        if (response.status === 200) {
            ...
            return { status: Status.Unreachable, msg: `Got ${response.status}: ${response.status}` }
        }
    }
}
