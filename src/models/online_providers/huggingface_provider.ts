...
export default class HuggingFaceProvider implements Model {
    ...
    reloadSettings(settings: LatexOCRSettings) {
        ...
        try {
            ...
            new Notice(`❌ There was an error loading your HuggingFace API key`)
            console.error('Error loading HuggingFace API key:', error);
            this.apiKey = ""
        }
    }

    load() {
        console.log("latex_ocr: HuggingFace API model loaded.")
    }
    ...
    async status() {
        if (this.apiKey === "") {
            return { status: Status.Misconfigured, msg: "HuggingFace API key required" }
        }
        ...
        try {
            ...
            return { status: Status.Ready, msg: "HuggingFace API key is working" }
        } catch (response) {
            if (response.status === 400 || response.status === 401) {
                return { status: Status.Misconfigured, msg: "Unauthorized: check your HuggingFace API key in the settings" }
            } else {
